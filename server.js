...
app.post("/send-monthly-report-mail", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Invalid 'startDate' or 'endDate' value" });
  }
...
const sendPPMReportMail = async (recipientEmail, timePeriod, firstName) => {
  const emailData = {
    sender: {
      email: "support@foxnetglobal.com",
      name: "Vyomika from Foxnet",
...
app.post("/send-feedback-mail", async (req, res) => {
  ...
  if (!ticketId || !firstName) {
    return res
      .status(400)
      .json({ message: "Ticket ID and first name are required" });
  }
...
const companySchema = new mongoose.Schema({
  ...
  // Company Coordinates (Latitude and Longitude)
  coordinates: {
  ...
  try {
    ...
    if (notifications.length === 0) {
      return res
        ...
        .json({
          message:
            "No new files uploaded in Health Check or PPM Check sections.",
        });
    }
...
app.delete(
  "/api/company/:companyName/contract/:mobileNumber",
  async (req, res) => {
    ...
    try {
      ...
      const contractIndex = company.contractMatrix.findIndex(
        (contact) => contact.phone === mobileNumber
      );
      if (contractIndex === -1) {
        return res
          .status(404)
          .json({ message: "Contract not found for the given phone number" });
      }
      ...
      res
        .status(200)
        .json({ message: "Contract matrix entry deleted successfully" });
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: "Server error" });
    }
...
app.post("/api/company/:companyName/contact", async (req, res) => {
  ...
  if (!title || !name || !email) {
    return res
      .status(400)
      .json({ message: "Title, name, and email are required." });
  }
...
app.post(
  "/upload/:checkType/:companyId",
  uploadPdf.array("file", 1),
  async (req, res) => {
    ...
    try {
      ...
      const checkFrequency =
        checkType === "healthcheck"
          ? company.healthCheck.frequency
          : company.ppmCheck.frequency;

      // Ensure only one file is uploaded for the current quarter/month
      if (files.length !== 1) {
        return res
          .status(400)
          .json({ error: "You need to upload exactly one PDF." });
      }
...
app.delete("/deleteFile/:companyId/:checkType/:fileId", async (req, res) => {
  ...
  try {
    ...
    const fileData = company[checkField].pdf.find(
    ...
    if (!fileData) {
      return res
        .status(404)
        .json({ error: "File not found for the specified ID" });
    }
    ...
    if (result.success) {
      ...
      company[checkField].pdf = company[checkField].pdf.filter(
        (file) => file._id.toString() !== fileId
      );
      await company.save();
      ...
      return res
        .status(200)
        .json({
          message: "File deleted successfully and removed from database",
        });
    } else {
    ...
    return res
      .status(500)
      .json({ error: "Error occurred while deleting the file" });
  }
});
...
app.post("/companies", async (req, res) => {
  const {
    ...
    ppmCheckFrequency,
    supportTimings,
    coordinates, // Receive coordinates (lat, lon)
  } = req.body;
  ...
  if (
    ...
    !healthCheckFrequency ||
    !ppmCheckFrequency ||
    !coordinates || // Ensure coordinates are provided
    !coordinates.lat || // Ensure latitude is provided
    !coordinates.lon // Ensure longitude is provided
  ) {
    return res.status(400).json({
      error:
        "Name, GST, address, health check frequency, ppm check frequency, and coordinates (lat, lon) are required.",
    });
  }
  ...
  try {
    const company = new Company({
      ...
      website,
      address,
      coordinates: { lat: coordinates.lat, lon: coordinates.lon }, // Save coordinates
      healthCheck: {
...
app.put("/notification/company/:companyName", async (req, res) => {
  try {
    ...
    if (operators.length === 0) {
      return res
        .status(404)
        .json({ message: "No operators found for this company" });
    }
...
const engineerSchema = new mongoose.Schema({
  ...
  // New fields for storing the real-time location
  location: {
    lat: { type: Number },
    lon: { type: Number },
  },
...
app.put("/engineers/location", async (req, res) => {
  const { mobile, lat, lon } = req.body;  // Expect mobile, lat, lon in the request body

  if (!mobile || !lat || !lon) {
    return res.status(400).send("Mobile number, Latitude, and Longitude are required.");
  }

  try {
    // Find the engineer by their mobile number
    const engineer = await Engineer.findOneAndUpdate(
      { mobile }, // Search for the engineer based on the mobile number
    ...
    res.json({ message: "Engineer location updated successfully.", engineer });
  } catch (err) {
    console.error("Error updating engineer location:", err);
...
app.get("/api/companies-with-tickets", async (req, res) => {
  try {
    // Get the current date and calculate the last 30 days
    const last30Days = moment().subtract(30, "days").toDate();

    // Aggregate to find companies that have created tickets in the last 30 days
    const companyTicketCounts = await Ticket.aggregate([
      {
        $match: {
          date: { $gte: last30Days },
        },
      },
      {
        $group: {
          ...
          ticketCount: { $sum: 1 }, // Count the number of tickets
        },
      },
    ]);

    // Send response with the company names and ticket counts
    res.json({
      success: true,
...
app.get("/operator-tickets", async (req, res) => {
  try {
    ...
    // Fetch the ticket names created in the last 30 days and group them by name
    const operators = await Ticket.aggregate([
      {
        $match: {
          date: { $gte: thirtyDaysAgo }, // Filter tickets created within the last 30 days
        },
      },
      {
        $group: {
          _id: "$name", // Group by the 'name' field (assuming this is the operator's name)
        },
      },
      {
        $project: {
          _id: 0, // Don't show the _id field
          operatorName: "$_id", // Use the grouped name as the operator's name
        },
      },
    ]);
...
app.get("/engineer-names-last-30-days", async (req, res) => {
  try {
    // Get the current date and calculate the date 30 days ago
    const currentDate = new Date();
    const thirtyDaysAgo = new Date(
    ...
    // Fetch engineers assigned to tickets created in the last 30 days and populate their details
    const engineers = await Ticket.aggregate([
      {
        $match: {
          date: { $gte: thirtyDaysAgo }, // Filter tickets created within the last 30 days
          engineerId: { $exists: true, $ne: null }, // Ensure there's an associated engineerId
        },
      },
      {
        $group: {
          _id: "$engineerId", // Group by engineerId (ensure engineers are distinct)
        },
      },
      {
        $lookup: {
          from: "engineers", // Lookup the 'engineers' collection to fetch engineer details
          localField: "_id", // Use engineerId as the local field
          foreignField: "_id", // Match it with the _id field in the engineers collection
          as: "engineerInfo", // Name the field that will hold engineer data
        },
      },
      {
        $unwind: "$engineerInfo", // Unwind the array to get a single engineer object per entry
      },
      {
        $project: {
          _id: 0, // Do not include the _id field
          engineerName: "$engineerInfo.name", // Display engineer's name
        },
      },
    ]);

    // Count the number of engineers (the length of the array)
    const engineerCount = engineers.length;

    // Send the result as response
    res.json({
      engineerCount: engineerCount,
      engineers: engineers.map((engineer) => engineer.engineerName), // Map over the array to get names
    });
  } catch (error) {
    console.error(error);
    res.status(500).send("Server Error");
...
app.get("/issue-category-names-last-30-days", async (req, res) => {
  try {
    // Get the current date and calculate the date 30 days ago
    const currentDate = new Date();
    const thirtyDaysAgo = new Date(
    ...
    // Fetch issue categories associated with tickets created in the last 30 days and populate their details
    const issueCategories = await Ticket.aggregate([
      {
        $match: {
          date: { $gte: thirtyDaysAgo }, // Filter tickets created within the last 30 days
          issueCategory: { $exists: true, $ne: null }, // Ensure there's an associated issueCategory
        },
      },
      {
        $group: {
          _id: "$issueCategory", // Group by issueCategory field
        },
      },
      {
        $lookup: {
          from: "issuecategories", // Lookup 'issuecategories' collection
          localField: "_id", // Match issueCategory (ObjectId) with _id in issuecategories
          foreignField: "_id",
          as: "issueCategoryInfo", // Add issue category details to the ticket
        },
      },
      {
        $unwind: "$issueCategoryInfo", // Unwind the array to get individual issue category data
      },
      {
        $project: {
          _id: 0, // Do not include the _id field
          issueCategoryName: "$issueCategoryInfo.name", // Show issue category name
        },
      },
    ]);

    const issueCategoryCount = issueCategories.length; // Count distinct issue categories

    res.json({
      issueCategoryCount: issueCategoryCount,
      issueCategories: issueCategories.map(
        (category) => category.issueCategoryName
...
// Helper function to get the current month in YYYY-MM format (e.g., "2025-02")
...
// Create a GET API route to fetch the number of open, closed tickets for the current month
app.get("/api/tickets/count", async (req, res) => {
  try {
    const currentMonth = getCurrentMonth();

    // Fetch open tickets (including In Progress) for the current month
    const openTicketsCount = await Ticket.countDocuments({
      status: { $in: ["Open", "In-Progress"] }, // Count both Open and In Progress tickets
      date: {
        $gte: new Date(`${currentMonth}-01T00:00:00.000Z`), // Start of the current month
        $lt: new Date(`${currentMonth}-31T23:59:59.999Z`), // End of the current month
      },
    });

    // Fetch closed tickets for the current month
    const closedTicketsCount = await Ticket.countDocuments({
      status: "Closed",
      closeDate: {
        $gte: `${currentMonth}-01`, // Start of the current month
        $lt: `${currentMonth}-31`, // End of the current month
      },
    });

    // Return the counts in the response
    res.json({
      openTickets: openTicketsCount, // This will include both Open and In Progress tickets
...
app.get("/user-monthly-eta/:contactNumber", async (req, res) => {
  try {
    const { contactNumber } = req.params;
    const currentMonth = getCurrentMonth();

    // Fetch tickets for the specific user (filtered by contact number and current month)
    const tickets = await Ticket.find({
      contactNumber: contactNumber,
      status: { $in: ["Closed"] }, // Only include "Closed" tickets
      date: {
        $gte: new Date(`${currentMonth}-01T00:00:00Z`),
        $lt: new Date(`${currentMonth}-28T23:59:59Z`),
      },
    });

    // Calculate total ETA for the user's closed tickets
    let totalHours = 0;
    let totalDays = 0;
    let exceeds24HoursCount = 0;

    tickets.forEach((ticket) => {
      totalHours += ticket.eta.totalHours || 0;
      totalDays += ticket.eta.totalDays || 0;
      if (ticket.eta.exceeds24Hours) {
        exceeds24HoursCount += 1;
      }
    });

    // Calculate average ETA for the user's closed tickets
    let averageHours = 0;
    let averageDays = 0;

    if (tickets.length > 0) {
      averageHours = totalHours / tickets.length;
      averageDays = totalDays / tickets.length;
    }
...
app.get("/admin-monthly-eta", async (req, res) => {
  try {
    const currentMonth = getCurrentMonth();

    // Fetch closed tickets for the current month (for all users) excluding "Open" and "In-Progress" tickets
    const tickets = await Ticket.find({
      status: { $in: ["Closed"] }, // Only include "Closed" tickets
      date: {
        $gte: new Date(`${currentMonth}-01T00:00:00Z`),
    ...
    // Calculate total ETA for closed tickets
    let totalHours = 0;
    let totalDays = 0;
    let exceeds24HoursCount = 0;
    ...
    // Calculate average ETA for closed tickets
    let averageHours = 0;
    let averageDays = 0;
...
app.get("/tickets/monthly-status", async (req, res) => {
  try {
    const currentYear = moment().year(); // Get current year
    const months = moment.months(); // Get an array of month names (January, February, ...)

    // Aggregate open tickets by month for the current year
    const openTickets = await Ticket.aggregate([
      {
        $addFields: {
          date: { $toDate: "$date" }, // Convert date field from string to Date
        },
      },
      {
        $match: {
      ...
      {
        $group: {
          _id: { $month: "$date" }, // Group by month of the date
          count: { $sum: 1 },
        },
      },
    ...
    // Aggregate closed tickets by month for the current year
    const closedTickets = await Ticket.aggregate([
      {
        $addFields: {
          closeDate: { $toDate: "$closeDate" }, // Convert closeDate field from string to Date
        },
      },
      {
        $match: { closeDate: { $ne: null } }, // Filter tickets that have a closeDate
      },
      {
        $group: {
          _id: { $month: "$closeDate" }, // Group by month of the closeDate
          count: { $sum: 1 },
        },
      },
      { $sort: { _id: 1 } }, // Sort by month (ascending)
    ...
    // Fill the result array with data for each month
    months.forEach((month, index) => {
      // Find the count of open and closed tickets for the current month
      const openCount = openTickets.find(
        (ticket) => ticket._id === index + 1
      ) || { count: 0 };
      const closedCount = closedTickets.find(
        (ticket) => ticket._id === index + 1
      ) || { count: 0 };
      ...
      result.push({
        ...
        closedTickets: closedCount.count,
      });
    });
...
// Route to get the number of tickets by category
app.get("/tickets/categorywise", async (req, res) => {
  try {
    const categories = ["CCTV", "Access Control", "Fire Alarm", "Pa system"];

    const categoryCount = await Ticket.aggregate([
      {
        $lookup: {
          from: "issuecategories", // Assuming your IssueCategory model is called 'issuecategories'
          localField: "issueCategory",
          foreignField: "_id",
          as: "categoryDetails",
        },
      },
      {
        $unwind: {
          ...
          preserveNullAndEmptyArrays: true, // Keep tickets that may not have a category
        },
      },
      {
        $group: {
          _id: {
            $cond: {
              if: {
                $in: ["$categoryDetails.name", categories], // Check if the category is one of the listed ones
              },
              then: "$categoryDetails.name",
              else: "Other",
            },
          },
          count: { $sum: 1 },
...
app.get("/ticket/:ticketId/feedback-status", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error retrieving feedback status",
        error: error.message,
      });
  }
});
...
app.put("/ticket/:ticketId/feedback-status", async (req, res) => {
  ...
  if (typeof feedbackSubmitted !== "boolean") {
    return res
      .status(400)
      .json({ message: "feedbackSubmitted must be a boolean" });
  }
...
app.get("/api/notifications/:mobileNumber", async (req, res) => {
  try {
    ...
    const tickets = await Ticket.find({
      ...
      .populate("engineerId"); // Populate additional fields as needed
    ...
    if (tickets.length === 0) {
      return res
        ...
        .json({
          message:
            "No tickets found for this mobile number in the last 2 days.",
        });
    }
  ...
  try {
    ...
    if (!tickets || tickets.length === 0) {
      return res
        .status(404)
        .json({
          message: `No tickets found for the company '${companyName}' in the date range ${startDate} to ${endDate}`,
        });
    }
    ...
    for (const category of issueCategories) {
      const categoryDoc = await IssueCategory.findOne({
        name: category,
      }).select("_id");
      ...
      return tickets
        .filter(
          (ticket) =>
            ...
            ticket.issueCategory.toString() === categoryName
        )
        .map((ticket) => {
          // If ETA is invalid (undefined, null, or empty), treat it as 0
          if (
            !ticket.eta ||
            ticket.eta.totalHours === undefined ||
            ticket.eta.totalHours === null
          ) {
    ...
    const accessControlCategory = await IssueCategory.findOne({
      name: "Access Control",
    }).select("_id");
    const fireAlarmCategory = await IssueCategory.findOne({
      name: "Fire Alarm System",
    }).select("_id");
    const cctvCategory = await IssueCategory.findOne({ name: "CCTV" }).select(
      "_id"
    );
    ...
    const accessControlETAs = calculateETAsForCategory(
      accessControlCategory._id.toString()
    );
    const fireAlarmETAs = calculateETAsForCategory(
      fireAlarmCategory._id.toString()
    );

    // Get the "Others" tickets (tickets that don't belong to any of the predefined categories)
    const othersETAs = tickets
      .filter((ticket) => {
        return (
          ticket.status === "Closed" &&
          ![
            cctvCategory._id.toString(),
            accessControlCategory._id.toString(),
            fireAlarmCategory._id.toString(),
    ...
    const monthlyETAHours = allETAs.reduce(
      (acc, eta) => acc + eta.totalHours,
      0
    );
    const monthlyETA =
      totalClosedTickets > 0 ? monthlyETAHours / totalClosedTickets : 0;
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching the ticket details." });
  }
});
...
app.get("/company/:companyName/checks", async (req, res) => {
  ...
  try {
    ...
    if (!company) {
      return res
        .status(404)
        .json({ message: "No company found with this name." });
    }
...
app.put("/companies/:id", async (req, res) => {
  try {
    ...
    // If coordinates are included in the update, make sure lat and lon are provided
    if (coordinates && (!coordinates.lat || !coordinates.lon)) {
      return res.status(400).json({
        error: "Both latitude and longitude are required to update coordinates.",
      });
    }
    ...
    const company = await Company.findByIdAndUpdate(
      req.params.id,
      {
        ...
        ...(coordinates && { coordinates }), // Update coordinates if provided
      },
      {
        new: true,
        runValidators: true,
...
app.get("/operators/by-company/:companyName", async (req, res) => {
  try {
    ...
    if (!company) {
      return res
        .status(404)
        .json({ message: `Company named ${companyName} not found.` });
    }
    ...
    if (operators.length === 0) {
      return res
        .status(404)
        .json({
          message: `No operators found for the company ${companyName}.`,
        });
    }
    ...
    res
      .status(500)
      .json({ message: `Error retrieving operators: ${err.message}` });
  }
});
...
app.post("/operators/mobile/:mobile/managers", async (req, res) => {
  ...
  // Validate input
  if (!name || !email || !designation || !contactNumber) {
  ...
  try {
    ...
    if (!operator) {
      return res
        .status(404)
        .json({ error: "No operator found with this mobile number." });
    }
    ...
    if (operator.managers && operator.managers.length >= 5) {
      return res
        .status(400)
        .json({ error: "Cannot add more than 5 managers." });
    }
...
///Delte manager
app.delete(
  "/operators/mobile/:mobile/managers/:managerMobile",
  async (req, res) => {
    ...
    try {
      ...
      operator.managers = operator.managers.filter(
        (manager) => manager.contactNumber !== managerMobile
      );
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const managerIndex = operator.managers.findIndex(
        (manager) => manager.contactNumber === managerMobile
      );
      if (managerIndex === -1) {
        return res
          .status(404)
          .json({ error: "Manager not found with this mobile number." });
      }
      ...
      operator.managers[managerIndex] = {
        ...
        contactNumber:
          contactNumber || operator.managers[managerIndex].contactNumber,
...
app.post("/engineers", async (req, res) => {   
  const { title, name, email, mobile } = req.body;    
  
  // Validate required fields
  if (!title || !name || !email || !mobile) {     
    return res.status(400).json({ 
      error: "Title, name, email, and mobile number are required." 
    });   
  }    

  try {     
    // Create new engineer with all possible fields from schema
    const engineer = new Engineer({
      title,          // required
      name,           // required
      email,          // required
      mobile,         // required
      contractType: req.body.contractType || "",    // optional
      managerName: req.body.managerName || "",      // optional
      rating: req.body.rating || 0                  // optional with default 0
    });     
    ...
    const ticketDetails = {
      ...
      eta: eta
        ? {
            ...
            exceeds24Hours:
              eta.exceeds24Hours !== undefined ? eta.exceeds24Hours : "N/A",
          }
...
app.put("/tickets/close/:ticketId", async (req, res) => {
  ...
  const formattedCloseDate = moment(closeDate, "YYYY-MM-DD").format(
    "YYYY-MM-DD"
  );
  ...
  try {
    ...
    const createdDateTimeString = `${
      createdDate.split("T")[0]
    }T${createdTime}:00.000Z`;
...
app.put("/tickets/in-progress/:ticketId", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message:
        "Ticket status set to 'In-Progress' and engineer updated successfully",
...
app.get("/tickets/open", async (req, res) => {
  try {
    ...
    const openInProgressTickets = await Ticket.find({
...
app.get("/tickets/closed", async (req, res) => {
  try {
    ...
    const ticketDetails = closedTickets.map((ticket) => ({
      ...
      date: ticket.date,
      description: ticket.description,
      // Include ETA details (you can customize this to include specific fields as needed)
      eta: ticket.eta
        ? {
            totalHours: ticket.eta.totalHours,
            totalDays: ticket.eta.totalDays,
            exceeds24Hours: ticket.eta.exceeds24Hours,
          }
        : null, // If no ETA is available, it will be null
    }));
    ...
    const response = await axios.post(
      ...
      formData,
      {
        headers: {
          ...formData.getHeaders(),
          "api-key":
            process.env.KALEYRA_API_KEY || "A17d7d416a4abf01de27c9dc4107272ec", // Your API key
        },
      }
    );
...
app.post("/send-whatsapp-inprogress", async (req, res) => {
  ...
  try {
    ...
    if (!sal || !name || !ticketId || !engineerName || !engineerMobile) {
      return res.status(400).json({
        message:
          "Missing required parameters (sal, name, ticketId, engineerName, engineerMobile)",
      });
    }
...
app.post("/send-whatsapp-closed", async (req, res) => {
  ...
  try {
    ...
    if (!sal || !name || !ticketId || !engineerName || !eta) {
      return res.status(400).json({
        message:
          "Missing required parameters (sal, name, ticketId, engineerName, eta)",
      });
    }
...
app.post("/send-whatsapp-assign-engineer", async (req, res) => {
  const {
    ...
    operatorName,
    operatorMobile,
  } = req.body; // Accept parameters from the request body
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Missing required parameters (engineerName, ticketId, companyName, operatorName, operatorMobile)",
      });
    }
...
app.post("/tickets/calculate-eta/:ticketId", async (req, res) => {
  ...
  try {
    ...
    if (isNaN(currentTime.getTime())) {
      return res
        .status(500)
        .json({ message: "Invalid current time on server." });
    }
    ...
    if (isNaN(differenceInMilliseconds)) {
      return res
        .status(500)
        .json({ message: "Error calculating time difference." });
    }
    ...
    if (isNaN(totalHours) || isNaN(totalDays)) {
      return res
        .status(500)
        .json({
          message: "Error calculating ETA. Invalid values for hours or days.",
        });
    }
...
app.get("/tickets/get-eta/:ticketId", async (req, res) => {
  ...
  try {
    ...
    if (!ticket.eta) {
      return res
        .status(404)
        .json({ message: "ETA not calculated for this ticket" });
    }
...
app.post("/check-send-whatsapp-message", async (req, res) => {
  const { to } = req.body; // 'to' is the number provided in the request

  // Log the received parameters for debugging
  console.log("Received parameters:", { to });

  // Define the allowed numbers
  const validNumbers = ["8859708414", "9560005265"];

  // Check if the entered number is in the allowed list
  if (!validNumbers.includes(to)) {
    return res
      .status(400)
      .json({ message: "Invalid number. Please enter a valid number." });
  }
...
app.get(
  "/tickets/category/:categoryName/mobile/:mobileNumber/closed",
  async (req, res) => {
...
app.post("/check-send-engineer-whatsapp-message", async (req, res) => {
  const { to } = req.body; // 'to' is the mobile number provided in the request

  // Log the received parameters for debugging
  console.log("Received parameters:", { to });

  // Check if the 'to' number is provided
  if (!to) {
    return res.status(400).json({ message: "Mobile number is required" });
  }

  try {
    // Check if the engineer exists in the database based on the mobile number
    const engineer = await Engineer.findOne({ mobile: to });
...
app.get("/tickets/engineer-issues", async (req, res) => {
  try {
    // Fetch all tickets where status is either 'Open' or 'In-Progress'
    const openInProgressTickets = await Ticket.find({
      status: { $in: ["Open", "In-Progress"] },
    })
      .populate("engineerId", "name") // Populating the engineer's name from the Engineer model
      .populate("issueCategory", "name") // Populating the issue category name
      .exec();

    // Map to extract only the required fields, including engineer's name and issue description
    const ticketDetails = openInProgressTickets.map((ticket) => ({
      ticketNo: ticket.ticketId,
      name: ticket.name,
      companyName: ticket.companyName,
      ...
      time: ticket.time,
      status: ticket.status,
      engineerName: ticket.engineerId ? ticket.engineerId.name : "N/A", // Engineer's name
      issueDescription: ticket.description, // Issue description
    }));
...
// Fetch tickets assigned to an engineer by mobile number
app.get("/api/tickets/engineer", async (req, res) => {
  ...
  try {
    // 1. Find the engineer by mobile number
    const engineer = await Engineer.findOne({ mobile });
    if (!engineer) {
      return res.status(404).json({ message: "Engineer not found." });
    }

    // 2. Fetch tickets assigned to this engineer using engineerId
    const engineerTickets = await Ticket.find({ engineerId: engineer._id })
      .populate("issueCategory")
      .populate("engineerId") // Populating engineer details
      .exec();

    if (engineerTickets.length === 0) {
      return res.status(404).json({ message: "No tickets found for this engineer." });
    }

    // 3. Return the tickets assigned to the engineer
    res.status(200).json(engineerTickets);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal server error." });
  }
});
app.get('/engineer-stats', async (req, res) => {
  ...
  try {
    // Step 1: Find the engineer by mobile number
    const engineer = await Engineer.findOne({ mobile });
    ...
    // Step 3: Fetch the tickets related to the engineer
    const tickets = await Ticket.find({ engineerId }).exec();

    // Step 4: Calculate the required statistics
    let totalOpenTickets = 0;
    let monthlyOpenTickets = 0;
    let totalClosedTickets = 0;
    let totalEta = 0;
    let closedTicketCount = 0;

    tickets.forEach(ticket => {
      // 1. Calculate Monthly Open Tickets (status = 'In-Progress' and created in the current month)
      if (ticket.status === 'In-Progress' && moment(ticket.date).isBetween(startOfMonth, endOfMonth, null, '[]')) {
        monthlyOpenTickets++;
      }

      // 2. Calculate Total Open Tickets (status = 'In-Progress' assigned to the engineer)
      if (ticket.status === 'In-Progress') {
        totalOpenTickets++;
      }

      // 3. Calculate Total Closed Tickets (status = 'Closed')
      if (ticket.status === 'Closed') {
        totalClosedTickets++;

        // 4. For ETA, calculate average for closed tickets (status = 'Closed')
        if (ticket.eta.totalHours && !isNaN(ticket.eta.totalHours)) {
          totalEta += ticket.eta.totalHours;
          closedTicketCount++;
        }
      }
    });

    // 5. Calculate the average ETA for closed tickets
    const averageEtaHours = closedTicketCount > 0 ? totalEta / closedTicketCount : 0;
    
    // Convert the ETA in hours to days
    const averageEtaDays = averageEtaHours / 24;
    ...
    return res.status(200).json({
      ...
      totalOpenTickets,
      totalClosedTickets,
      averageEtaHours,  // ETA in hours
      averageEtaDays,   // ETA in days
    });
  } catch (error) {
    console.error(error);
...
// Helper function to calculate estimated time based on distance (in km)
const calculateEstimatedTime = (distanceKm, speedKmPerHour = 50) => {
  const timeInHours = distanceKm / speedKmPerHour;
  return timeInHours; // in hours
};


// API to calculate the estimated time and distance
app.post('/api/engineer-estimate', async (req, res) => {
  const { mobile, companyName } = req.body;

  if (!mobile || !companyName) {
    return res.status(400).json({ message: 'Mobile number and company name are required.' });
  }

  try {
    // Fetch engineer's data based on the mobile number
    const engineer = await Engineer.findOne({ mobile });
    ...
    // Fetch company's data based on the company name
    const company = await Company.findOne({ name: companyName });
    ...
    const companyLocation = company.coordinates; // Coordinates stored in 'coordinates' in company schema

    if (!engineerLocation || !companyLocation) {
      return res.status(400).json({ message: 'Both engineer and company must have valid coordinates.' });
    }

    // Reverse geocoding to get the engineer's location name
    let engineerLocationName = null;
    if (engineerLocation.lat && engineerLocation.lon) {
      try {
        const response = await axios.get(`https://nominatim.openstreetmap.org/reverse`, {
          params: {
            lat: engineerLocation.lat,
            lon: engineerLocation.lon,
            format: 'json',
          },
        });
        engineerLocationName = response.data.display_name || 'Location not found';
      } catch (error) {
        engineerLocationName = 'Error fetching location name';
      }
    }

    // Calculate the distance using geolib's distance function
    const distanceKm = geolib.getDistance(
      { latitude: engineerLocation.lat, longitude: engineerLocation.lon },
      { latitude: companyLocation.lat, longitude: companyLocation.lon }
    ) / 1000; // Convert to km

    // Calculate the estimated time (in hours)
    const estimatedTime = calculateEstimatedTime(distanceKm);

    // Return the result with coordinates and location names
    return res.json({
      engineerCoordinates: engineerLocation,
      engineerLocationName,
      companyCoordinates: companyLocation,
      distanceKm,
...
// API to fetch ticket details and calculate estimated time and distance
app.post('/api/operator-ticket-estimate', async (req, res) => {
  ...
  try {
    // Fetch operator's data based on the mobile number
    const operator = await Operator.findOne({ mobile });
    ...
    // Fetch tickets associated with the operator's company and status "In-Progress"
    const tickets = await Ticket.find({ companyName: operator.companyName, status: "In-Progress" });

    if (!tickets || tickets.length === 0) {
      return res.status(404).json({ message: 'No tickets found in progress for this operator.' });
    }

    // Get the first ticket's engineerId (assuming there is at least one)
    const ticket = tickets[0];
    const engineerId = ticket.engineerId;

    // Fetch engineer's data to get the coordinates and name
    const engineer = await Engineer.findById(engineerId);

    if (!engineer) {
      return res.status(404).json({ message: 'Engineer not found for this ticket.' });
    }

    const engineerLocation = engineer.location;
    const engineerName = engineer.name; // Add engineer's name here
    if (!engineerLocation) {
      return res.status(400).json({ message: 'Engineer does not have valid location.' });
    }

    // Fetch company data to get the company coordinates
    const company = await Company.findOne({ name: operator.companyName });

    if (!company) {
      return res.status(404).json({ message: 'Company not found.' });
    }

    const companyLocation = company.coordinates;
    if (!companyLocation) {
      return res.status(400).json({ message: 'Company does not have valid coordinates.' });
    }
    ...
    // Return the result with ticket no, engineer's name, coordinates, and estimated time
    return res.json({
      ticketNumber: ticket.ticketId, // Added ticket number
      engineerName: engineerName, // Added engineer's name
      engineerCoordinates: engineerLocation,
      engineerLocationName,
      companyCoordinates: companyLocation,
      distanceKm,
