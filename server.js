...
app.get("/issue-category-names-last-30-days", async (req, res) => {
  try {
    ...
    res.json({
      ...
      issueCategories: issueCategories.map(category => category.issueCategoryName), // List of category names
    });
  } catch (error) {
    console.error(error);
    res.status(500).send("Server Error");
...
app.get('/admin-monthly-eta', async (req, res) => {
  try {
    ...
    const tickets = await Ticket.find({
      status: { $in: ["Closed"] }, // Only include "Closed" tickets
      date: { $gte: new Date(`${currentMonth}-01T00:00:00Z`), $lt: new Date(`${currentMonth}-28T23:59:59Z`) },
    });
  ...
  try {
    ...
    months.forEach((month, index) => {
      ...
      const openCount = openTickets.find(ticket => ticket._id === index + 1) || { count: 0 };
      const closedCount = closedTickets.find(ticket => ticket._id === index + 1) || { count: 0 };
...
app.get("/tickets/open", async (req, res) => {
  ...
  if (!validNumbers.includes(to)) {
    return res.status(400).json({ message: "Invalid number. Please enter a valid number." });
  }
